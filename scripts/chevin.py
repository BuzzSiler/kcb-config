...
def DoTakeRequest(payroll_number, filename):
    try:
        return_code, box, transaction_number = fleetwave.TakeKeyReservation(client, 
                                                                            kiosk, 
                                                                            payroll_number)
        if return_code == 1:
...
def DoReturnRequest(payroll_number, filename):
    try:
        return_code, box, transaction_number = fleetwave.ReturnKeyReservation(client, 
                                                                              kiosk, 
                                                                              payroll_number)
        if return_code == 1:
...
if __name__ == "__main__":
    ...
    if take_return == 'take' and request_complete == 'request':
        payroll_number = args[2]
        result = DoTakeRequest(payroll_number, TRANSACTION_FILE)
    ...
    #   On failure, return nothing
        ...
        payroll_number = args[2]
        result = DoReturnRequest(payroll_number, TRANSACTION_FILE)
        if result == "failed":
            sys.stdout.write(result)
            sys.exit(1)
        elif result == "error":
